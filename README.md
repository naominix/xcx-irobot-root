...
Xcratchでは、数値欄を選ぶとRootの動きを図で確認できるモーションピッカーが開きます。左右モーター出力、移動距離、回転角、円弧の半径・角度は、図をマウスまたは指でドラッグして指定できます。パネル内の数値欄から精密な値を直接入力でき、iPadでは数字キーボードまたは画面内テンキーも利用できます。従来どおり演算ブロックの差し込みも可能です。
