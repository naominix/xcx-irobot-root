...
RGB値による点灯に加え、点滅と回転アニメーションを指定できます。音はScratch標準のピアノ鍵盤から音階を選ぶか、上級者向けの周波数（Hz）で指定できます。鍵盤を押すと、接続中のRootで短く試聴できます。音ブロックはRootから同じpacket IDの再生完了応答を受信してから次へ進み、通知が失われた場合だけ指定時間の1秒後に安全に処理を再開します。マーカー／消しゴムは上げ下げブロックで操作します。
