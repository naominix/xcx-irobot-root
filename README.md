...
接続状態が不整合になった場合は、「すべてのRootを切断して接続状態をリセットする」ブロックを一度実行してください。すべてのBLE接続と受信済みセンサー値を初期化し、Root1・Root2などの番号スロットは残します。保存済みプロジェクトのRoot選択ブロックを壊さず、次の接続で既存番号を再利用するためです。
